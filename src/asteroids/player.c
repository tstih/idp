...
void get_direction(struct player* p, struct vector2d *direction) {

	memcpy(direction, &(p->obj_vert[0]),sizeof(struct vector2d));
...
void shoot_bullet(struct player* p) {
	...
	for (i = 0; i < BULLETS; i++) {
		...
		if (p->bullets[i].alive == FALSE) {
			...
			memcpy(&(p->bullets[i].location), &(p->world_vert[0]),sizeof(struct vector2d));
...
void update_player(struct player* p) {
	...
	for (i =0; i < P_VERTS; i++) {
		...
		add_vector_new(&p->obj_vert[i], &p->location, &(p->world_vert[i]));	
		add_vector(&p->world_vert[i], &translation);
	}
