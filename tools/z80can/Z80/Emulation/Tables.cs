namespace Z80.Emulation
{
	static public class Tables
	{
		...
		public const byte FLAG_P = 0x04;
		public const byte FLAG_V = FLAG_P;
		public const byte FLAG_3 = 0x08;
		public const byte FLAG_H = 0x10;
		public const byte FLAG_5 = 0x20;
		...
		internal readonly static byte[] halfcarry_add_table = { 0, FLAG_H, FLAG_H, FLAG_H, 0, 0, 0, FLAG_H };
		internal readonly static byte[] halfcarry_sub_table = { 0, 0, FLAG_H, 0, FLAG_H, 0, FLAG_H, FLAG_H };
		internal readonly static byte[] overflow_add_table = { 0, 0, 0, FLAG_V, FLAG_V, 0, 0, 0 };
		internal readonly static byte[] overflow_sub_table = { 0, FLAG_V, 0, 0, 0, 0, FLAG_V, 0 };
		internal readonly static byte[] sz53_table, parity_table, sz53p_table;

		#region DAA table
		/*table for daa, contains af*/
		internal readonly static byte[] daatab = {
		...
		static Tables()
		{
			sz53_table = new byte[0x100];
			parity_table = new byte[0x100];
			sz53p_table = new byte[0x100];

			for (int i = 0; i < 0x100; i++)
			{
				sz53_table[i] = (byte)(i & (FLAG_3 | FLAG_5 | FLAG_S));
				var j = i; byte parity = 0;
				int k;
				for (k = 0; k < 8; k++) { parity ^= (byte)(j & 1); j >>= 1; }
				parity_table[i] = parity != 0 ? (byte)0 : FLAG_P;
				sz53p_table[i] = (byte)(sz53_table[i] | parity_table[i]);
			}
