namespace Z80.Emulation
{
	...
	{
		OperationDelegate[] _opcodes_base;

		void InitializeOpcodesBase()
		{
			_opcodes_base = new OperationDelegate[] {
		...
		void op_0x02()
		{
			LD_A_TO_ADDR_MPTR(ref _tmpbyte, Registers.A, Registers.BC);
			CPUWriteMemory((Registers.BC), _tmpbyte);
		...
		void op_0x0a()
		{
			_tmpbyte = CPUReadMemory(Registers.BC);
			LD_A_FRO_ADDR_MPTR(ref Registers.A, _tmpbyte, Registers.BC);
		...
		void op_0x12()
		{
			LD_A_TO_ADDR_MPTR(ref _tmpbyte, Registers.A, Registers.DE);
			CPUWriteMemory((Registers.DE), _tmpbyte);
		...
		void op_0x1a()
		{
			_tmpbyte = CPUReadMemory(Registers.DE);
			LD_A_FRO_ADDR_MPTR(ref Registers.A, _tmpbyte, Registers.DE);
		...
		void op_0x2a()
		{
			...
			LD_RP_FRO_ADDR_MPTR_16(ref Registers.HL, Registers.TD, Registers.TA);
		...
		void op_0x32()
		{
			Registers.TAl = READ_OP();
			Registers.TAh = READ_OP();
			LD_A_TO_ADDR_MPTR(ref _tmpbyte, Registers.A, Registers.TA);
			CPUWriteMemory(Registers.TA, _tmpbyte);
		...
		void op_0x34()
		{
			_tmpbyte = CPUReadMemory(Registers.HL);
			INC(ref _tmpbyte);
			CPUWriteMemory(Registers.HL, _tmpbyte);
		...
		void op_0x3a()
		{
			Registers.TAl = READ_OP();
			Registers.TAh = READ_OP();
			_tmpbyte = CPUReadMemory(Registers.TA);
			LD_A_FRO_ADDR_MPTR(ref Registers.A, _tmpbyte, Registers.TA);
